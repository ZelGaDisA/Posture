...
export const GridForResults = ({resultSideNumber, image, rerenderCounter}) => {
    ...
    const drawSoloPatientGrid = (db, size) => {
        ...
        function drawGrid(size) {
            ...
            const getRotatedLine = (start, angle, t) => {
                let x1 = start.x + t * Math.cos(angle);
                let y1 = start.y + t * Math.sin(angle);
                let x2 = start.x - t * Math.cos(angle);
            ...
            {//Рисует человека
                ...
                const MAN_height = coords.ankles.center.y - coords.ears.left.y +
                    ((coords.ears.left.x - coords.ears.center.x) * 3) + "px"
